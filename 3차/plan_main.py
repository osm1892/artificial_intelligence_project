...
class PlanningProblem:
    ...
    def expand_fluents(self, name=None):
        ...
        if name is not None:
            ...
            fluent_list = list(map(lambda fluent: Expr(fluent[0], *fluent[1]), {fluent.op: fluent.args for fluent in
                                                                                self.initial + self.goals + [clause for
                                                                                                             action in
                                                                                                             self.actions
                                                                                                             for clause
                                                                                                             in
    ...
    def is_strips(self):
        ...
        return (all(clause.op[:3] != 'Not' for clause in self.goals) and all(
            clause.op[:3] != 'Not' for action in self.actions for clause in action.precond))
...
def make_spaghetti():
    return PlanningProblem(initial='~Clean(Pan) & Have(Onion) & Have(SpaghettiNoodle) & Have(Tomato) & Have(Ketchup)',
                           goals='Have(Spaghetti)',
                           actions=[Action(action='Set(tool)', precond='~Clean(tool)', effect='Clean(tool)'),
...
def main():
    ...
    solution = [expr('Set(Pan)'), expr('MakeSauce(Pan, Onion, Tomato, Ketchup, SpaghettiSauce)'),
                expr('MakeNoodle(SpaghettiNoodle)'), expr('MakePasta(SpaghettiSauce, SpaghettiNoodle, Spaghetti)')]
